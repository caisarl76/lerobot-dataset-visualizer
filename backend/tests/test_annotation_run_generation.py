...
def workflow(tmp_path, monkeypatch):
    ...
    atomic_json(root / "meta/lerobot_annotations.json", {"version": 2, "episodes": labels})
    ...
    def state_for(request):
        state_root = (
            Path(request.local_path) if request.local_path else Path(app._local_aliases()[request.repo_id])
        )
        saved = json.loads((state_root / "meta/lerobot_annotations.json").read_text())["episodes"]
        return SimpleNamespace(
            root=state_root,
            info={"codebase_version": "v3.1"},
            episodes_df=pd.DataFrame({"episode_index": [0, 1, 2]}),
            annotations={int(ep): SimpleNamespace(atoms=value["atoms"]) for ep, value in saved.items()},
        )
    ...
    def generate(source, output, annotations, config, episodes, **kwargs):
        ...
        assert len(episodes) == 1, "Run orchestration must checkpoint one official target at a time"
        episode = episodes[0]
        ...
        sidecar["episodes"][str(episode)]["atoms"][0]["content"] = f"generated {episode}"
        ...
        atomic_json(prediction, {"episodes": {str(episode): sidecar["episodes"][str(episode)]}})
        return {
            "output_dir": str(output),
            "first_generated_episode_index": episode,
            "validation": {"ok": True, "errors": [], "warnings": [], "episodes_checked": 3},
            "prediction_snapshot": str(prediction),
            "phases": [],
            "episode_results": {str(episode): {"generation_status": "generated", "issues": []}},
        }

    engine = SimpleNamespace(parse_config=parse_config, generate_dataset=generate, REVISION=REVISION)
    monkeypatch.setattr(app, "_official_engine", lambda: engine)
    return SimpleNamespace(
        root=root, store=store, run=run, calls=calls, fail=fail, persisted_before_episode=persisted_before_episode
    )


def test_good_episode_checkpoint_survives_next_failure_and_later_episode_continues(workflow):
    workflow.fail.add(1)
    queued = app.create_annotation_job(app.GenerationRequest(local_path=str(workflow.root)))
    ...
    assert workflow.calls[1]["source"] == workflow.calls[2]["source"] == first_checkpoint
    ...
    labels = json.loads((root / "meta/lerobot_annotations.json").read_text())["episodes"]
    ...
    predictions = [json.loads(path.read_text()) for path in (root / "meta/annotation_predictions").glob("*.json")]
    assert all("1" not in prediction["episodes"] for prediction in predictions)
    assert any(
        prediction.get("episode_results", {}).get("1", {}).get("generation_status") == "failed"
        for prediction in predictions
    )


def test_restart_marks_job_interrupted_and_resume_selects_only_unfinished_episodes(workflow):
    ...
    restored_labels = json.loads((current_root / "meta/lerobot_annotations.json").read_text())
    assert restored_labels["episodes"]["0"]["atoms"][0]["content"] == "completed before restart"
    assert (current_root / snapshot.relative_to(workflow.root)).read_bytes() == original_prediction
    assert current["current_job_id"] == queued["job_id"] != interrupted_job
    assert app.get_annotation_job(interrupted_job)["status"] == "interrupted"
    with pytest.raises(HTTPException) as error:
        app.create_annotation_job(app.GenerationRequest(local_path=current["root"], resume_unfinished=True))
    assert error.value.status_code in {409, 422}
    assert [call["episode"] for call in workflow.calls] == [1, 2]


def test_revision_change_between_capture_and_admission_rejects_stale_generation(workflow, monkeypatch):
    ...
    with pytest.raises(HTTPException) as error:
        app.create_annotation_job(app.GenerationRequest(local_path=str(workflow.root), task_prompt="stale task"))
